#!/usr/bin/env node
/**
 * TV-EV-01 — replay-consistency walk of the SMA100 gate.
 * BINDING pre-registration: docs/PREREG_TV-EV-01.md (params fixed at 4ce8c0a,
 * decidability amendments at 23dfe6c). No parameter may move.
 *
...
async function walkSymbol(sym, outDir) {
  ...
  const tester = (cell.trades || []).filter((t) => t.entry_time >= winStartMs && t.entry_time <= WINDOW_END_MS);
  const walker = trades.filter((t) => t.entry_time >= winStartMs && t.entry_time <= WINDOW_END_MS);
  ...
  const result = {
    ...
    tester_trades_in_scope: tester.length, walker_trades: walker.length,
...
const outDir = process.argv.includes('--out') ? process.argv[process.argv.indexOf('--out') + 1] : '/home/karim/claude-a15-20260818/strategy-tournament/tv-ev-01';
...
try {
  ...
  writeFileSync(join(outDir, 'TV-EV-01-verdict.json'), JSON.stringify({ verdict, passes, results: results.map(({ matches: _m, walker_trade_list: _w, ...rest }) => rest) }, null, 2));
} finally {
